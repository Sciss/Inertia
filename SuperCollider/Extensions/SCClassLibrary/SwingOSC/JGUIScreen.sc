	...
	*viewPalette {
	...
	refresh {
		if( drawHook.isNil, {
			...
			JPen.protRefresh(ÊdrawHook, view, server, penID, this.id );
		});
	}
...
//		server.sendMsg( '/set', this.id, \background, *(Color( 1, 1, 1, alpha ).asSwingArg) );
	...
	*prGetScreenBounds { arg argBounds, server;
	 ...
	 */
	initSCWindow { arg argName, argBounds, resizable, border, argServer;
...
//		id			= view.id;
	...
	prSetName { arg argName;
		server.sendMsg( '/set', this.id, \title, argName.asSwingArg );
	}
