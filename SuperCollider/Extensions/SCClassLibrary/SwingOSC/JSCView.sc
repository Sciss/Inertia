 ...
 *	@todo		missing classes : SCMultiSliderView subclasses
...
JSCView {  // abstract class
	...
	*initClass {
	...
	/**
	 *	Creates a new instance of this view. The 'argServer' and 'id'
	 *	arguments are only used in SwingOSC, not in cocoa GUI. Usually
	 ...
	 *	@param	argServer	(SwingOSC) the server to use or 'nil' to use SwingOSC.default
	 *	@param	id		(Integer) the SwingOSC node id or 'nil' to automatically create one
	 */
	*new { arg parent, bounds, argServer, id;
		^super.new.init( parent, bounds, argServer, id );
	}
	...
	*paletteExample { arg parent, bounds;
	...
	init { arg argParent, argBounds, argServer, id;
		parent = argParent.asView;	// actual view
		this.prInit( parent, argBounds.asRect, this.class.viewClass, argServer, id );
	...
	focus { arg flag = true;
		if( flag, {
			...
			"JSCView.focus( false ) : not yet implemented".error;
		});
	}
	...
	properties {
		^#[ \bounds, \visible, \enabled, \canFocus, \resize, \background,
		    \minWidth, \maxWidth, \minHeight, \maxHeight, \opaque ] // JJJ: opaque
	}
	...
	prUpdateBounds { arg newBounds;
		properties.put( \bounds, newBounds );
	}
	...
	prCreateCompResponder { arg bndl;
		...
		cmpResp = OSCpathResponder( server.addr, [ '/component', this.id ], { arg time, resp, msg;
			...
			state = msg[2].asSymbol;
			case { (state === \moved) || (state === \resized) }
			{
...
//					(value + jinsets.top + jinsets.bottom).asSwingArg );
	...
	*importDrag { 
	...
	prBoundsToJava { arg rect;
		...
		pb = parent.jinsets.subtractFrom( parent.bounds );
	...
	prBoundsFromJava { arg rect;
		...
		pb = parent.jinsets.addTo( parent.bounds );
...
JSCTopView : JSCContainerView {	// NOT subclass of JSCCompositeView
	...
	*new { arg window, bounds, argServer, id;  // JJJ
		^super.new( nil, bounds, argServer, id ).prInitTopView( window );
	}
...
//		var childID;
//		
//		if( child.jinsets.notNil, {
//			childID = if( child.prIsInsideContainer, { "cn" ++ child.id }, { child.id });
//			server.sendBundle( nil, [ '/method', id, \putClientProperty, "insets" ] ++ child.jinsets.asSwingArg );
//		});
//		^super.add( child );
//	}
...
//							\minorTickSpacing, value, \extent, value );
		...
		{ key === \bounds }
		{
// prBoundsToJava will be called by super!!
//			value = this.prBoundsToJava( value );
			if( if( value.width > value.height, 0, 1 ) != orientation, {
...
//("sum = "++sizes.sum).postln;
		if( (sizes.sum + 70) < 8192, {
			server.listSendMsg([ '/set', this.id, \listData, '[', '/array' ] ++ items ++
...
//		font = argFont;
	...
	prSCViewNew {
		...
		acResp = OSCpathResponder( server.addr, [ '/list', this.id ], { arg time, resp, msg;
			...
			newVal = this.prFixValue( msg[4] );
			// ignore -1 values which happen during dnd
			if( (msg[4] >= 0) and: { newVal != this.value }, {
				// don't call valueAction coz we'd create a loop
				properties.put( \value, newVal );
				{ this.doAction; }.defer;
			});
...
JSCUserView : JSCView {	var <>keyDownFunc, <drawFunc;
	...
	prUpdateBounds { arg newBounds;
		super.prUpdateBounds( newBounds );
...
//		string		= string.asString;
		...
		// 8192 - sizeof([ '/method', 1234, \setString, 0, 1, "" ])
		if( string.size <= 8148, {
			...
			while({ off < string.size }, {
				len = min( string.size - off, 8192 - bndlSize );
...
JSCEnvelopeView : JSCAbstractMultiSliderView {
	...
	properties {
		^super.properties ++ #[ \font, \selectedIndex, \clipThumbs ];  // \lastIndex
	}
...
//	var <fixedSelection = false;
	...
	value_ { arg val;
		var oldSize, xvals, yvals, valClip;
...
//		^this.getProperty( \value, axy );
		...
		^Array.with( value[0].copy, value[1].copy );
	}
...
//			});
	...
	prFixValues {
		...
		step	= this.step;
		if( step > 0, {
			val.do({ arg xyvals; xyvals.size.do({ arg i; xyvals[ i ] = xyvals[ i ].round( step ).clip( 0.0, 1.0 )})});
		}, {
			val.do({ arg xyvals; xyvals.size.do({ arg i; xyvals[ i ] = xyvals[ i ].clip( 0.0, 1.0 )})});
		});
	}
