...
//		serverRunning	= false;
		...
		helloResp		= OSCresponderNode( addr, '/swing', { arg time, resp, msg;
			var pingAddr;
			if( msg[ 1 ].asSymbol === \hello, {
...
//					this.initTree;
		...
		this.initTree({ this.serverRunning = true });
	}
	...
	initTree { arg onComplete, onFailure;
		...
		try {
			this.connect;
			{
				...
				if( this.sync, onComplete, onFailure );
			}.fork( clock );
		}
	...
	sync { arg condition, bundles, latency, timeout = 4.0;
		...
		queryMsg			= [ '/query', id, '[', '/field', 'de.sciss.swingosc.SwingOSC', \VERSION, ']' ];
	...
	sendMsgSync { arg msg, successMsg, failedMsg, timeout = 4.0, condition;
		...
		respDone	= OSCresponderNode( addr, successMsg[ 0 ], { arg time, resp, msg;
			...
			successMsg.do({ arg c, i;
				if( msg[ i ].isKindOf( Symbol ), {
					if( msg[ i ] !== c.asSymbol, {Êok = false; });
				}, {
					if( msg[ i ] != c, {Êok = false; });
				});
			});
		...
		if( timeout > 0.0, {
			cancel = {
				timeout.wait;
				respDone.remove;
				if( respFailed.notNil, {ÊrespFailed.remove; });
				result			= nil;
				condition.test	= true;
				condition.signal;
	...
	reboot { arg func; // func is evaluated when server is off
		...
		if( serverRunning, {
			...
			this.boot;
				...
				// really dumb workaround to get a sign of life / response
	status {		// maybe get something more interesting back from server?
