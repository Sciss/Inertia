    ...
    *new { arg filename; // type String         
    ...
    parse { arg parentNode, pos; // types DOMNode, int          
        ...
        if ( pos < endIndex, {
            if ( nodeValue[pos] == $< , { // tag
                ...
                //trimmed = Toolbox.trim( n.getData );
    ...
    parseQuoted { arg pos, result; // types int, Ref          
        ...
        if ( (q.ascii != 34) && (q.ascii != 39), { // " or '
            ("XML parser: quoted value (starting with ' or \") expected in file " ++ this.filename ++ ", line " ++ this.linenr(pos)).die;	// \"
        });
        s = "";
        esc = false;
        pos = pos + 1;
