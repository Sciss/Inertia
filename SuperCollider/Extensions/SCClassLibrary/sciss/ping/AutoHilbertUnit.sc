 ...
 */
AutoHilbertUnit : GeneratorUnit {
	var defName	= nil;
	var synth		= nil;
	var amount	= 1.0;
	
	classvar <allUnits;				// elem = instances of AutoHilbertUnit
	...
	*new { arg server;
		^super.new( server ).prInitAutoHilbertUnit;
	}
	
	prInitAutoHilbertUnit {
		allUnits.add( this );
	...
	setInputBus { arg bus;
		...
		defName		= ("autoHilbertUnit" ++ numChannels).asSymbol;
	...
	prCacheDef { arg numChannels;
		...
		if( defCache.contains( defName ).not, {
			def = SynthDef( defName, { arg in, out, amount = 1.0;
				...
				ReplaceOut.ar( out, (inp * (1.0 - amount)) + (flt * amount) );
			}, [ nil, nil, 0.1 ]);
			defCache.add( def );
		});
	}
