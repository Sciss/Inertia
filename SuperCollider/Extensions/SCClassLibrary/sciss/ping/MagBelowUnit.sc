 ...
 */
MagBelowUnit : GeneratorUnit {
	var defName = nil, thresh = 1.0e-1;
	var bufs = nil, synth = nil;
	var bufSize;
	
	classvar <allUnits;			// elem = instances of MagBelowUnit
	...
	*new { arg server;
		^super.new( server ).prInitMagBelowUnit;
	}
	...
	prInitMagBelowUnit {
		...
		attributes		= [
			UnitAttr( \thresh, ControlSpec( 1.0e-2, 1.0e-0, \exp ), \normal, \getThresh, \setThresh, nil, false )
		];
...
//bndl.postln;
	...
	setInputBus { arg bus;
		...
		defName		= ("magBelowUnit" ++ numChannels).asSymbol;
...
//				ramp			= EnvGen.kr( env );
				...
				ReplaceOut.ar( out, volume * IFFT( chain ) );
			}, [ nil, nil, nil, 0.1 ]);
			defCache.add( def );
		});
	}
