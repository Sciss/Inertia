...
// ZZZ
//				this.protAddGlobalBuffer( buf );
				bndl.add( this.prAllocConsecutiveMsg( bufs, synth.newMsg( target, [Ê\aBuf, bufs.first.bufnum, \speed1, speed1, \speed2, speed2, \in, inBus.index, \out,
					outBus.notNil.if({ outBus.index }, 0 ) ], \addToHead )));
			}, {
				bndl.add( synth.newMsg( target, [Ê\aBuf, bufs.first.bufnum, \speed1, speed1, \speed2, speed2, \in, inBus.index, \out,
					outBus.notNil.if({ outBus.index }, 0 ) ], \addToHead ));
			});
//bndl.postln;
