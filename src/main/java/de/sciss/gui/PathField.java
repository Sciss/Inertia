...
// -------- public Methoden --------
	...
	public PathField( int type, String dlgTxt )
	{
		...
		if( (type & TYPE_OUTPUTFILE) != 0 ) {
			icnWriteProtected	= new MutableIcon();
			icnWriteProtected.setID( MutableIcon.WRITE_PROTECTED );
			lbWarn				= new JLabel( icnWriteProtected );
			con.gridx++;
//lbWarn.setText( "HA!" );
			lay.setConstraints( lbWarn, con );
			add( lbWarn );
		} else {
			icnWriteProtected = null;
		}
...
//		if( (fPath != null) && (fPath.length() > 0) ) {
			try {
				exists	= path.isFile();
				wp		= !path.canWrite() || ((path.getParentFile() != null) && !path.getParentFile().canWrite());
...
//		}
		...
		if( icnWriteProtected != null ) {
			icnWriteProtected.setID( wp ? MutableIcon.WRITE_PROTECTED : MutableIcon.INVISIBLE );
		}
	}
