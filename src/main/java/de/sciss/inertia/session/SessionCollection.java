 ...
 *
 *  Changelog:
 *		07-Aug-05	copied from de.sciss.eisenkraut.session.SessionCollection
 *					; added add( Object, int, SessionObject )
 *		14-Aug-05	allows null event sources
 */
...
implements EventManager.Processor
{
	...
	private final java.util.Set	dynamicSet	= new HashSet();
	...
	public void debugDump( int indent )
	{
		...
		for( int i = 0; i < collObjects.size(); i++ ) {
			...
			if( so instanceof AbstractSessionObject ) {
				((AbstractSessionObject) so).debugDump( indent + 1 );
			} else {
				printIndented( indent + 1, "object "+(i+1)+" = "+so.getName()+
					" (" + so.getClass().getName() + ")" );
			}
		}
//		elm.debugDump();
