 ...
 *
 *  Changelog:
 *		07-Aug-05	copied from de.sciss.eisenkraut.util.MapManager
 *		17-Aug-05	new debugDump ; added getValueAvecDefault()
 *		30-Sep-05	lazy elm creation
 */
...
//System.err.println( "put default value '"+context.defaultValue+"' for '"+key+"'" );
			} else {
	...
	{
//System.err.println( "putValue source = "+(source == null ? "null" : source.getClass().getName())+" ; key = "+key+ "; value = "+value );
...
//System.err.println( "elm.dispatchEvent "+key );
...
//System.err.println( "removeValue source = "+source.getClass().getName()+" ; key = "+key );
		...
		if( (result != null) && (source != null) && (elm != null) ) {
...
//System.err.println( "putAllValues source = "+source.getClass().getName() );
	...
	/**
	 *	Returns the map value. If not found,
	 *	either the provided default is returned,
	 *	or if the provided default is null,
	 *	the context's default is returned.
	 *	If no context default exists in this case,
	 *	null is returned.
	 */
	public Object getValueAvecDefault( String key, Object defaultValue )
	{
		final Object val = backingMap.get( key );
		if( val != null ) return val;
		if( defaultValue != null ) return defaultValue;
		
		final Context c = getContext( key );
	...
	public void clearValues( Object source )
	{
		...
		boolean dispatch	= !keySet.isEmpty() && (source != null) && (elm != null);
	...
	{
		printIndented( indent, "map ; owner = "+owner.getClass().getName()+" . contains : " );
		Iterator iter = backingMap.keySet().iterator();
		String key, val;
		while( iter.hasNext() ) {
			key = iter.next().toString();
			val	= backingMap.get( key ).toString();
			printIndented( indent + 1, "key = "+key+" ; val = "+val );
		}
	}
...
loop:	for( i = 0; i < nl.getLength(); i++ ) {
		...
		if( elm != null ) elm.dispatchEvent( new MapManager.Event( this, this, keySet ));
	}
